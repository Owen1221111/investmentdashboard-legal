//
//  USStockLoanSyncSelectionView.swift
//  InvestmentDashboard
//
...
struct USStockLoanSyncSelectionView: View {
    @Environment(\.managedObjectContext) private var viewContext
    @Environment(\.dismiss) private var dismiss

    let client: Client
    let stocks: [USStock]
    let onSync: ([USStock], Loan) -> Void

    @State private var selectedStocks: Set<USStock> = []
        ...
        NavigationView {
            VStack(spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Text("選擇與貸款相關的美股標的,將同步到「貸款/投資月度管理」")
                        .font(.caption)
    ...
    // MARK: - 標的選擇行
    private func stockSelectionRow(stock: USStock) -> some View {
        Button(action: {
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    HStack(spacing: 12) {
                        Text("市值: $\(formatNumber(stock.marketValue))")
                            .font(.caption)
                            .foregroundColor(.secondary)
...
#Preview {
    ...
    return USStockLoanSyncSelectionView(
        client: client,
        stocks: [],
        onSync: { _, _ in }
    )
