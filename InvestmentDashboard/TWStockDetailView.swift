...
struct TWStockDetailView: View {
        ...
        .sheet(isPresented: $showingColumnReorder) {
            ColumnReorderView(
                ...
                onSave: { newOrder in
                    ...
                    // 儲存到 UserDefaults
                    UserDefaults.standard.set(newOrder, forKey: "TWStock_ColumnOrder")
                }
            )
        }
        .onAppear {
            // 從 UserDefaults 讀取欄位排序
            if let savedOrder = UserDefaults.standard.array(forKey: "TWStock_ColumnOrder") as? [String], !savedOrder.isEmpty {
    ...
    // MARK: - 台股表格
    private var twStockTable: some View {
            ...
            VStack(spacing: 0) {
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    ...
                    HStack(spacing: 0) {
                        ...
                        ForEach(currentColumnOrder, id: \.self) { header in
                            if header == "成本" {
                                Text(formatNumberWithCommas(String(format: "%.2f", calculateTotalCost())))
                                ...
                                Text(formatNumberWithCommas(String(format: "%.2f", calculateTotalMarketValue())))
                                    .font(.system(size: 15, weight: .bold))
                                    .foregroundColor(.primary)
                                    .padding(.horizontal, 8)
                                    .padding(.vertical, 12)
    ...
    private func calculateTotalCost() -> Double {
        return twStocks.reduce(0.0) { total, stock in
            let costStr = removeCommas(stock.cost ?? "")
            return total + (Double(costStr) ?? 0)
        }
    }

    private func calculateTotalMarketValue() -> Double {
        return twStocks.reduce(0.0) { total, stock in
            let marketValueStr = removeCommas(stock.marketValue ?? "")
            return total + (Double(marketValueStr) ?? 0)
        }
    }

    private func calculateTotalProfitLoss() -> Double {
        return twStocks.reduce(0.0) { total, stock in
            let profitLossStr = removeCommas(stock.profitLoss ?? "")
            return total + (Double(profitLossStr) ?? 0)
        }
    }
    ...
    // MARK: - 資料綁定
    private func bindingForStock(_ stock: TWStock, header: String) -> Binding<String> {
        Binding<String>(
            get: {
                let rawValue: String
                switch header {
                case "日期":
                    ...
                    rawValue = stock.createdDate.map { dateFormatter.string(from: $0) } ?? ""
    ...
    // 取得報酬率顏色
    private func getReturnRateColor(_ stock: TWStock) -> Color {
    ...
    private func deleteStock(_ stock: TWStock) {
        withAnimation {
            viewContext.delete(stock)
            do {
                try viewContext.save()
                PersistenceController.shared.save()
                print("✅ 刪除台股並同步到 iCloud")
            } catch {
                print("❌ 刪除失敗: \(error)")
            }
        }
